...
import { sbe_string_is_valid_table_filter } from "./sb-table-filters.js";
export class SandboxTableFilterEditorForm extends FormApplication {
  ...
  static get defaultOptions() {
    ...
    const overrides = {
      height: '600',
      width:'625',
      id: 'sbe-table-filter-editor-form',
      template: `modules/sandbox-extensions/templates/sbe-table-filter-editor-form.hbs`,
      title: _title,
      userId: game.userId,
  ...
  activateListeners(html) {
    ...
    //
    html.find('.sbe-table-filter-editor-move-row-up').click(this._moveRowUp.bind(this));
    html.find('.sbe-table-filter-editor-move-row-down').click(this._moveRowDown.bind(this));
    html.find('.sbe-table-filter-editor-delete-row').click(this._deleteRow.bind(this));
    html.find('.sbe-table-filter-editor-add-row').click(this._addRow.bind(this));
    
    
    html.find('.sbe-table-filter-editor-type').change(this._conditionTypeChange.bind(this));
    html.find('.sbe-table-filter-editor-key').change(this._conditionKeyChange.bind(this));
  }
  ...
  getData(options) {     
    ...
    let filtertable='';    
    let filter = sbe_string_is_valid_table_filter(this.expression);    
    if (filter!=null){
      filter.conditions.forEach(function (condition) {
        filtertable += _buildInputRow(condition);      
      });
    }        
    data={
  ...
  async _conditionKeyChange(event){
    ...
    // get type
    const type=type_td_for_this_row.getElementsByClassName('sbe-table-filter-editor-type')[0].value;
    switch(type){
      case 'citem':
        switch(key){
          case 'isactive':
      ...
      case 'attribute':
        ...
        let property=game.items.find(y=>y.type=="property" && y.data.data.attKey==key);
        if (property!=null){          
          switch(property.data.data.datatype){
            case 'checkbox':
              value_td_for_this_row.innerHTML=_build_table_value_select_boolean('');
              break;
            case 'list':
              // get option list
              let list= property.data.data.listoptions;
  ...
  async  _conditionTypeChange(event){
    ...
    switch(event.target.value){
      case 'citem':
        ...
        key_td_for_this_row.getElementsByClassName('sbe-table-filter-editor-key')[0].addEventListener('change',e=>{this._conditionKeyChange(e);});
        value_td_for_this_row.innerHTML=_build_value_input('');
        break;
      case 'table':
        // changes the key input to a select
      ...
      case 'attribute':
        ...
        key_td_for_this_row.innerHTML=_build_type_attribute_keys_input('');
        key_td_for_this_row.getElementsByClassName('sbe-table-filter-editor-key')[0].addEventListener('change',e=>{this._conditionKeyChange(e);});
        value_td_for_this_row.innerHTML=_build_value_input('');
        break;
    }
  ...
  async _addRow(event){    
    ...
    // add event listerner
    //html.find('.sbe-table-filter-editor-key').change(this._conditionKeyChange.bind(this));
    cellkey.getElementsByClassName('sbe-table-filter-editor-key')[0].addEventListener('change',e=>{this._conditionKeyChange(e);});
    ...
    // add event listener
    cellmanipulators.getElementsByClassName('sbe-table-filter-editor-move-row-up')[0].addEventListener('click',e=>{this._moveRowUp(e);});
    cellmanipulators.getElementsByClassName('sbe-table-filter-editor-move-row-down')[0].addEventListener('click',e=>{this._moveRowDown(e);});
    cellmanipulators.getElementsByClassName('sbe-table-filter-editor-delete-row')[0].addEventListener('click',e=>{this._deleteRow(e);});
...
function _assembleJSONFilter(){
  let sfilter='';
  let filtertable=document.getElementById('sbe-table-filter-editor-data-tbody');      
  if (filtertable!=null){
    let sLogic;
    let sType;
    let sKey;
    ...
    if (filtertable.rows.length>1){
      ...
      for (let i = 0, row; i<filtertable.rows.length-1; i++) {
         ...
         sValue  =  filtertable.rows[i].cells[4].getElementsByClassName('sbe-table-filter-editor-value')[0].value;
         // assemble json row
...
function _buildInputRow(condition){
    let returnvalue='';
    let htmlinput='';
    // add for logic
    htmlinput+=`<td>`;
    ...
    if(condition.type=='citem'){      
      ...
      htmlinput+=`<td>`;
      htmlinput+=_build_type_attribute_keys_input(condition.key);      
      htmlinput+='</td>';
    }
    ...
    if(condition.type=='citem'){
      ...
      // attribute
      let property=game.items.find(y=>y.type=="property" && y.data.data.attKey==condition.key);
      if (property!=null){
        switch(property.data.data.datatype){
          case 'checkbox':
            htmlinput+=_build_table_value_select_boolean(condition.value);
  ...
  function _build_manipulators() {
    let returnvalue = '';
    returnvalue += `<i class="sbe-table-filter-editor-move-row-up fas fa-arrow-alt-circle-up" title="Move up"></i>
    <i class="sbe-table-filter-editor-move-row-down fas fa-arrow-alt-circle-down" title="Move down"></i>
    <i class="sbe-table-filter-editor-delete-row fas fa-trash" title="Delete condition"></i>`;       
    return returnvalue;
  }
  
  function _build_value_input(currentvalue) {
    let returnvalue = '';
    returnvalue += `<input type="text" value="`+ currentvalue +`" class="sbe-table-filter-editor-value"/>`;       
    return returnvalue;
  }
  ...
  function _build_operator_select(currentvalue){
    let returnvalue = '';
    returnvalue+=`<select class="sbe-table-filter-editor-operator">`;
  ...
  function _build_type_select(currentvalue){
    let returnvalue = '';
    returnvalue+=`<select class="sbe-table-filter-editor-type">`;
    returnvalue+=_buildOption('citem',currentvalue);
    returnvalue+=_buildOption('attribute',currentvalue);
  ...
  function _build_logic_select(currentvalue){
    let returnvalue = '';
    returnvalue+=`<select class="sbe-table-filter-editor-logic">`;
  ...
  function _build_type_citem_keys_select(currentvalue) {
    let returnvalue = '';
    returnvalue += `<select class="sbe-table-filter-editor-key">`;
  ...
  function _build_type_attribute_keys_input(currentvalue) {
    let returnvalue = '';
    returnvalue += `<input type="text" value="`+ currentvalue +`" class="sbe-table-filter-editor-key"/>`;       
    return returnvalue;
  }
  
  function _build_table_value_select_boolean(currentvalue) {
    let returnvalue = '';
    returnvalue += `<select class="sbe-table-filter-editor-value">`;
  ...
  function _build_table_value_datalist_from_list(id,list,currentvalue) {
    let returnvalue = '';
    returnvalue += `<input list="sbe-table-filter-editor-value-datalist-`+id+`" id="sbe-table-filter-editor-value-`+id+`" class="sbe-table-filter-editor-value" value="`+currentvalue+`"/>`;
    returnvalue += `<datalist id="sbe-table-filter-editor-value-datalist-`+id+`">`;
    // convert to array
    let listArr = list.split(',');
    for (let i = 0; i < listArr.length; i++) {      
      returnvalue += _buildOption(listArr[i], currentvalue);
