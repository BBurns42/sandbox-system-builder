...
export class gActorSheet extends ActorSheet {
    ...
    activateListeners(html) {
        ...
        html.find('.item-delete').click(async(ev) => {
            ...
            const bOkToProceed=await sb_custom_dialog_confirm('Confirm deletion?',`This will delete <b>${stabs[todelete].name}</b><br>Do you want to proceed?<br>` ); 
            if(bOkToProceed){
        ...
        html.find('.citem-delete').click(async(ev) => {
            ...
            // ask user for confirmation
            const bOkToProceed=await sb_custom_dialog_confirm('Confirm deletion?',`This will delete <b>${item.name}</b><br>Do you want to proceed?<br>` ); 
            if(bOkToProceed){            
    ...
    */
    ...
    async generateRollDialog(dialogID, dialogName, rollexp, rollname, rollid, actorattributes, citemattributes, number, isactive, ciuses, rollcitemID, targets, useData) {
        ...
        if (dialogPanel.system.type == "multipanel") {
...
`;
            ...
            if (panelProperty.system.datatype != "table" && panelProperty.system.datatype != "textarea" && panelProperty.system.datatype != "badge" && !panelProperty.system.ishidden) {
...
`;
    ...
    async setTemplate(gtemplate, actorData) {
        console.log("Sandbox | setTemplate | Setting sheet for " + this.actor.name);
        ...
        for (let n = 0; n < properties.length; n++) {
            ...
            if (propertybase == null) {
                ...
                if (property.system.haslabel && property.system.datatype != "table" && property.system.datatype != "badge" && property.system.datatype != "button") {
                    ...
                    else if (property.system.labelformat == "D") {
                        sLabel.textContent = "";
                        ...
                        if ((propTable.system.onlynames == "DEFAULT" || propTable.system.onlynames == "ONLY_NAMES") && !isFree) {
                            ...
                            if(propTable.system.tableoptions.showicons){
                              ...
                              if(citem!=null){
                                ...
                                firstcell.innerHTML = auxMeth.sb_two_col_card('<img src="' + citemimg +'" class="sb-citem-table-icon" /> ',  ciObject.name);
                              } else {
                        ...
                        if (propTable.system.onlynames == "NO_NAMES" && !isFree) {
                          ...
                          if(propTable.system.tableoptions.showicons){
                            ...
                            if(citem!=null){
                              ...
                              firstcell.className += " " + inputgroup;
                              firstcell.innerHTML = auxMeth.sb_two_col_card('<img src="' + citemimg +'" class="sb-citem-table-icon" /> ',  "");
                              firstcell.setAttribute("item_id", ciObject.id);
                              firstcell.setAttribute("item_ciKey", ciObject.ciKey);
                              firstcell.addEventListener("click", this.linkCItem, false);
                              new_row.appendChild(firstcell);
                                        ...
                                        if (propdata.datatype != "label")
                                            if (!isFree) {
                                                ...
                                                if (checknonumsum) {
                                                    ...
                                                    constantvalue = await auxMeth.autoParser(constantvalue, this.actor.system.attributes, ciObject.attributes, justexpr, false, ciObject.number, ciObject.uses);
                                                }
                                            }
                                            else {
...
//                                            }
                ...
                if (propTable.system.hastotals && table != null) {
                    let new_row = document.createElement("TR");
...
<input class="check-transfer" type="checkbox" id="transfer-all">
    ...
    async deleteCItem(itemID, cascading = false,askforconfirmation=false) {
      ...
      if(askforconfirmation){
        const item=game.items.get(itemID);
        // ask user for confirmation
        bOkToProceed=await sb_custom_dialog_confirm('Confirm deletion?',`This will delete <b>${item.name}</b><br>Do you want to proceed?<br>` );
      } 
      if(bOkToProceed){ 
                ...
                if (attributes[attKey] != null) {
                    ...
                    if (maxRadios > 0) {
                        for (let j = 0; j <= parseInt(maxRadios); j++) {
                            ...
                            if (property.system.editable || game.user.isGM)
